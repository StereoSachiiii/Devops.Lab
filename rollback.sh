...
usage() {
  echo "Usage: ./rollback.sh <service-name> <previous-image-or-tag>"
  echo "Example: ./rollback.sh core-service v1.0.4"
  echo "Example: ./rollback.sh web-frontend devops/web-frontend:sha-abc1234"
  echo ""
...
# 1. Export override for target service image
export ${ENV_VAR_NAME}="$TARGET_IMAGE"

# 2. Re-create and start only the target service without touching Tier 0 or other services
echo "🚀 Deploying rolled-back container..."
docker compose -f $COMPOSE_FILE --env-file $ENV_FILE up -d --no-deps "$SERVICE_NAME"

# 3. Wait for service to report healthy
