...
export async function mfaRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post(
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      await req.prisma.user.update({ where: { id: sub }, data: { mfaSecret: secret } });
  ...
  fastify.post(
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      await req.prisma.user.update({ where: { id: sub }, data: { mfaEnabled: true } });
  ...
  fastify.post(
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        const user = await req.prisma.user.findUnique({ where: { id: decoded.sub } });
        if (!user || !user.mfaSecret) {
